...
PS_OUT main(VsOut pin)
{
    ...
    //float4 color = decale_texture.Sample(samplerStates[_LINEAR_WRAP_SAMPLER_INDEX], decalTexcoord) * pin.color;
    //clip(color.a < 0.5f ? -1 : 1);
    
    //float4 normal = decaleNormalTexture.Sample(samplerStates[_LINEAR_WRAP_SAMPLER_INDEX], decalTexcoord);
