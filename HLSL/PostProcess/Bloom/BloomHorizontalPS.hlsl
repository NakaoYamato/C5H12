...
float4 main(VsOut pin) : SV_TARGET
{
    ...
    for (int i = 1; i < 3; i++)
    {
        sampled_color += hdr_color_buffer_texture.Sample(linearBorderBlackSampler, pin.texcoord + float2(offset[i] / width, 0.0)) * weight[i];
        sampled_color += hdr_color_buffer_texture.Sample(linearBorderBlackSampler, pin.texcoord - float2(offset[i] / width, 0.0)) * weight[i];
    }
#else
	...
	for (int i = 0; i < 2; i++)
	{
		sampled_color += hdr_color_buffer_texture.Sample(linearBorderBlackSampler, pin.texcoord + float2(offset[i], 0.0) / width) * weight[i];
		sampled_color += hdr_color_buffer_texture.Sample(linearBorderBlackSampler, pin.texcoord - float2(offset[i], 0.0) / width) * weight[i];
	}
#endif
