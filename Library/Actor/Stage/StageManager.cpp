...
void StageManager::OnStart()
{
	...
	for (auto& layout : _createdEnvironmentLayouts)
	{
		...
		if (layout.parent != "")
		{
			auto parentActor = GetScene()->GetActorManager().FindByName(layout.parent);
			if (parentActor)
			{
				actor->ReleaseParent();
...
void StageManager::OnUpdate(float elapsedTime)
{
	...
	if (_isEditing)
	{
		...
		auto& view = GetScene()->GetMainCamera()->GetView();
		auto& projection = GetScene()->GetMainCamera()->GetProjection();
		...
		auto actors = GetScene()->GetCollisionManager().RayCast(rayStart, rayDir, distance);
		if (actors.size() > 0)
		{
			// このオブジェクト以外と交差していたら交差点を保存
			for (auto& result : actors)
			{
				if (result.hitActor != this)
				{
...
// 環境アクター生成
std::shared_ptr<Actor> StageManager::CreateEnvironmentActor(
	...
	const Vector3& scale)
{
	...
	{
		auto parentActor = GetScene()->GetActorManager().FindByName(parent);
		if (parentActor)
		{
			actor->SetParent(parentActor.get());
...
// 編集用GUI描画
void StageManager::DrawEditingGui()
{
	auto actorFactory = _actorFactory.lock();
	if (!actorFactory)
		return;
