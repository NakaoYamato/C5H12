...
public:
	...
	struct Option
	{
		...
		// 選択可能かのコールバック名
		std::string canSelectCallbackName{};
		// 選択時コールバック名
...
protected:
	...
	// ラベルの選択不可時の色
	Vector4 _optionLabelDisabledColor = Vector4::Gray;
...
// チェックボックスウィジェット
// 選択可能かのコールバック関数を用いて
class MenuCheckBoxWidget : public MenuWidget
{
public:
	MenuCheckBoxWidget(std::string name) : MenuWidget(name) {}
	~MenuCheckBoxWidget() override = default;
	// ウィジェット名取得
	virtual std::string GetWidgetName() const { return ClassToString<MenuCheckBoxWidget>(); }
...
protected:
	// チェストボックスのON画像
	Sprite _checkedOptionSprite{ L"./Data/Texture/UI/CheckBoxOn.png", Sprite::CenterAlignment::CenterCenter };
	// チェストボックスのOFF画像
	Sprite _uncheckedOptionSprite{ L"./Data/Texture/UI/CheckBoxOff.png", Sprite::CenterAlignment::CenterCenter };
};
