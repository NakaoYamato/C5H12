...
void DebugCamera::Update(float elapsedTime)
{
    ...
    if (Debug::Input::IsActive(DebugInput::BTN_F4))
    {
        ...
        // マウスの移動距離算出
        float moveX = _INPUT_IS_MOVED("MouseMoveX") * _movePower;
        float moveY = _INPUT_IS_MOVED("MouseMoveY") * _movePower;
        ...
        {
            // ズーム
            _eye += _zoomPower * front * _INPUT_IS_MOVED("MouseOldWheel") / 60.0f;
        }
