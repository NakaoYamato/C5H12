...
{
	_ASSERT_EXPR(bufferCount <= MAX_GBUFFER_COUNT, L"The maximum number of buffers has been exceeded.");
	...
	for (UINT buffer_index = 0; buffer_index < bufferCount; ++buffer_index)
	{
		...
		hr = device->CreateRenderTargetView(render_target_buffer.Get(), &render_target_view_desc, &rtvs[buffer_index]);
		_ASSERT_EXPR(SUCCEEDED(hr), HRTrace(hr));
		...
		hr = device->CreateShaderResourceView(render_target_buffer.Get(), &shader_resource_view_desc, &renderTargetSRVs[buffer_index]);
		_ASSERT_EXPR(SUCCEEDED(hr), HRTrace(hr));
	}
	...
	shader_resource_view_desc.ViewDimension = D3D11_SRV_DIMENSION_TEXTURE2D;
	shader_resource_view_desc.Texture2D.MipLevels = 1;
	hr = device->CreateShaderResourceView(depth_stencil_buffer.Get(), &shader_resource_view_desc, &depthStencilSRV);
	_ASSERT_EXPR(SUCCEEDED(hr), HRTrace(hr));
...
void GBuffer::Deactivate(ID3D11DeviceContext* immediateContext)
{
	...
	immediateContext->OMSetRenderTargets(bufferCount, cachedRTVs, cachedDSV);
	for (UINT buffer_index = 0; buffer_index < bufferCount; ++buffer_index)
	{
		if (cachedRTVs[buffer_index])
		{
			cachedRTVs[buffer_index]->Release();
		}
	}
	if (cachedDSV)
	{
