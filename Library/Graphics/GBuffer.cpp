	...
	bufferCount_(bufferCount)
{
	...
	for (UINT buffer_index = 0; buffer_index < bufferCount; ++buffer_index)
	{
		...
		hr = device->CreateRenderTargetView(render_target_buffer.Get(), &render_target_view_desc, &rtvs_[buffer_index]);
		_ASSERT_EXPR(SUCCEEDED(hr), HRTrace(hr));
		...
		hr = device->CreateShaderResourceView(render_target_buffer.Get(), &shader_resource_view_desc, &renderTargetSRVs_[buffer_index]);
		_ASSERT_EXPR(SUCCEEDED(hr), HRTrace(hr));
	}
