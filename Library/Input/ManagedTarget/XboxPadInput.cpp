...
{
	// 使用するボタンを設定
	_states[XINPUT_GAMEPAD_DPAD_UP] = FALSE;
	_states[XINPUT_GAMEPAD_DPAD_RIGHT] = FALSE;
	_states[XINPUT_GAMEPAD_DPAD_DOWN] = FALSE;
	_states[XINPUT_GAMEPAD_DPAD_LEFT] = FALSE;

	_states[XINPUT_GAMEPAD_A] = FALSE;
	_states[XINPUT_GAMEPAD_B] = FALSE;
	_states[XINPUT_GAMEPAD_X] = FALSE;
	_states[XINPUT_GAMEPAD_Y] = FALSE;

	_states[XINPUT_GAMEPAD_START] = FALSE;
	_states[XINPUT_GAMEPAD_BACK] = FALSE;

	_states[XINPUT_GAMEPAD_LEFT_SHOULDER] = FALSE;	// L1
	_states[XINPUT_GAMEPAD_LEFT_THUMB] = FALSE;		// L3
	_states[XINPUT_GAMEPAD_RIGHT_SHOULDER] = FALSE;	// R1
	_states[XINPUT_GAMEPAD_RIGHT_THUMB] = FALSE;		//R3

	_values[XBOXPAD_AXIS_LX] = 0.0f;
	_values[XBOXPAD_AXIS_LY] = 0.0f;
	_values[XBOXPAD_AXIS_RX] = 0.0f;
	_values[XBOXPAD_AXIS_RY] = 0.0f;
...
void XboxPadInput::Update()
{
	...
	if (XInputGetState(_slot, &xinputState) == ERROR_SUCCESS)
	{
		...
		// 入力量がある情報の更新
		_values[XBOXPAD_AXIS_LX] = static_cast<float>(pad.sThumbLX) / static_cast<float>(0x8000);
		_values[XBOXPAD_AXIS_LY] = static_cast<float>(pad.sThumbLY) / static_cast<float>(0x8000);
		_values[XBOXPAD_AXIS_RX] = static_cast<float>(pad.sThumbRX) / static_cast<float>(0x8000);
		_values[XBOXPAD_AXIS_RY] = static_cast<float>(pad.sThumbRY) / static_cast<float>(0x8000);

		_values[XBOXPAD_TRIGGER_L] = static_cast<float>(pad.bLeftTrigger) / 255.0f;
		_values[XBOXPAD_TRIGGER_R] = static_cast<float>(pad.bRightTrigger) / 255.0f;
	}
}

/// ゲームパッドのキー番号から文字列に変換
const char* XboxPadInput::ToString(int vKey)
{
	switch (vKey)
	{
