...
#include <imgui.h>
#include <imgui_internal.h> // ImGui::GetWindowDrawList() 用

void AnimationCurve::DrawGui()
{
    auto& keyframes = GetKeyframes();

	if (ImGui::Button("Add Keyframe")) {
		// 新しいキーフレームを追加 (例: 時間0.5、値1.0)
		AddKeyframe(0.5f, 1.0f);
	}
    ...
    if (isMouseOverCanvas && ImGui::IsMouseClicked(1)) { // 1は右クリック
        ...
        AddKeyframe(newKeyPos.x, newKeyPos.y);
        // 新しく追加したものを選択状態にする
        selectedKeyframe = FindKeyframeIndex(newKeyPos.x, newKeyPos.y);
    }
