...
{
	// クールタイムが終了したら遷移
	if (!_owner->GetEnemy()->IsAttackCooldown())
	{
...
void WeakMoveState::OnExecute(float elapsedTime)
{
	...
	if (!isTargetInRange)
	{
		float jogSpeed = _owner->GetWeak()->GetJogSpeed();
		// ターゲット方向に移動
		_owner->GetEnemy()->MoveToTarget(targetPosition, jogSpeed);
	}
	else
	{
		// ターゲットが攻撃範囲内なら移動しない
		_owner->GetEnemy()->GetCharactorController()->SetVelocityXZ(0.0f, 0.0f);
	}
...
void WeakDashState::OnExecute(float elapsedTime)
{
	...
	if (!isTargetInRange)
	{
		float dashSpeed = _owner->GetWeak()->GetDashSpeed();
		// ターゲット方向に移動
		_owner->GetEnemy()->MoveToTarget(targetPosition, dashSpeed);
	}
	else
	{
		// ターゲットが攻撃範囲内なら移動しない
...
void WeakJumpAttackState::OnEnter()
{
	_owner->GetAnimator()->PlayAnimation(
		...
		false,
		0.5f);
	_owner->GetAnimator()->SetRootNodeIndex("Raptor_Hub001");
