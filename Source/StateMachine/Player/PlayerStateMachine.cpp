...
void PlayerStateMachine::Execute(float elapsedTime)
{
    ...
    if (_player->GetSustainedDamage() > 0)
    {
        ...
        {
            if (_player->IsGuard())
                _stateMachine.ChangeState("GuardBreak");
            else
                _stateMachine.ChangeState("HitKnockDown");
        }
        else
        {
            if (_player->IsGuard())
                _stateMachine.ChangeState("GuardHit");
            else
...
void PlayerIdleState::OnExecute(float elapsedTime)
{
	...
	// ガード移行
		...
		owner->GetStateMachine().ChangeState("Guard");
}
#pragma endregion
...
namespace GuardSubState
{
	...
	public:
		...
		void OnExecute(float elapsedTime) override
		{
            // アニメーションが終了していたら遷移
            if (!owner->GetAnimator()->IsPlayAnimation())
                owner->GetStateMachine().ChangeSubState("Guarding");
		}
		void OnExit() override
		{
		}
